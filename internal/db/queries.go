...
`
	...
	if opts.LocalOnly {
		query += " AND (SELECT r.ZLOCALAVAILABILITY FROM ZINTERNALRESOURCE r WHERE r.ZASSET = a.Z_PK AND r.ZRESOURCETYPE = 0 ORDER BY r.ZDATALENGTH DESC LIMIT 1) = 1"
	...
	if opts.LocalOnly {
		countQuery += " AND (SELECT r.ZLOCALAVAILABILITY FROM ZINTERNALRESOURCE r WHERE r.ZASSET = a.Z_PK AND r.ZRESOURCETYPE = 0 ORDER BY r.ZDATALENGTH DESC LIMIT 1) = 1"
	} else if opts.CloudOnly {
		countQuery += " AND (SELECT r.ZLOCALAVAILABILITY FROM ZINTERNALRESOURCE r WHERE r.ZASSET = a.Z_PK AND r.ZRESOURCETYPE = 0 ORDER BY r.ZDATALENGTH DESC LIMIT 1) = -1"
	}
	if opts.PhotosOnly {
